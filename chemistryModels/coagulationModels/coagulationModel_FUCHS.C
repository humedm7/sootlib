...
double coagulationModel_FUCHS::getCoagulationSootRate(const state& state,
    ...
    double d1 = std::pow(6.*m1/(M_PI*rhoSoot), onethird);
    double d2 = std::pow(6.*m1/(M_PI*rhoSoot), onethird);
    ...
    double c1 = std::sqrt(8.*kb*state.T/(M_PI * m1));
    double c2 = std::sqrt(8.*kb*state.T/(M_PI * m2));

    double l1 = 8.*D1/(M_PI*c1);
    double l2 = 8.*D2/(M_PI*c2);

    double g1 = root2/(3.*d1*l1) * ( std::pow(d1+l1, 3.0) - std::pow(d1*d1 + l1*l1, 1.5) ) - root2*d1;  // Seinfeld text is missing the root2 in the last term
    double g2 = root2/(3.*d2*l2) * ( std::pow(d2+l2, 3.0) - std::pow(d2*d2 + l2*l2, 1.5) ) - root2*d2;

    double K12 = 2.*M_PI*(D1 + D2)*(d1 + d2) / 
                 ( (d1 + d2)/(d1 + d2 + 2.*std::sqrt(g1*g1 + g2*g2)) + 
                   8./(eps_c*FM_multiplier)*(D1 + D2)/(std::sqrt(c1*c1 + c2*c2)*(d1 + d2)) 
                 );
    return K12;
}
