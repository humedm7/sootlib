...
double nucleationModel_AJ_RED::getNucleationSootRate(state &state) {
    ...
    double da = 1.395E-10 * std::sqrt(3); // diameter of single aromatic ring

    dtar = da * std::sqrt(2*state.mtar/(3*12.011));

    Bt   = dtar*dtar * std::sqrt(8*M_PI*kb*state.T/state.mtar);


    const double Rnuc = eps*Bt*state.tarVar[0]*state.tarVar[0];    // kmol/m3*s
