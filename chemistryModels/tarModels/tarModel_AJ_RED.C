...
double tarModel_AJ_RED::getCrackingTarRate(state &state) {
    ...
    double r2 = k2*xphe;
    double r3 = 50/128*k3*xnapth*std::pow(state.getGasSpC(gasSp::H2), 0.4);
    double r4 = 14/92*k4*xtol*std::pow(state.getGasSpC(gasSp::H2), 0.5);
    double r5 = k5*xben;
...
double tarModel_AJ_RED::getDepositionTarRate(state &state) {
    ...
    double d_a  = std::sqrt(1.395E-10 * 3);                                // diameter of a single aromatic ring
    double dsoot = std::pow(6*msoot/(M_PI*rhoSoot), onethird);  // diameter of average soot particle
    double dtar  = d_a * std::sqrt(2*state.mtar/(3*12.011));               // diameter of average tar particle

    if (state.sootVar[1] > 0) {
        muTS = state.mtar*msoot/(state.mtar + msoot);
        Bts  = std::pow((dsoot + dtar), 2) * std::sqrt(M_PI*kb*state.T/(2*muTS));
    }
...
double tarModel_AJ_RED::getSurfaceTarRate(state &state) {
    ...
    double term1 = 1.92E-3 * std::sqrt(state.getGasSpP(gasSp::CO2))*state.T*state.T*exp(-5.55E6/(Rg*state.T));
    double term2 = 1.92E-3 * std::pow(state.getGasSpP(gasSp::H2O), 1.21) / std::sqrt(state.T) * exp(-4.17E8/(Rg*state.T));
