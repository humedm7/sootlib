...
void sootModel_MOMIC::setSourceTerms(state &state) {
    ...
    const double Acoef = M_PI * std::pow(6./(M_PI*rhoSoot), twothird);
...
void sootModel_MOMIC::downselectIfNeeded(vector<double> &M) {
    ...
    if (M[1] <= 0.0)
        M[1] = M[0] * mavg * exp(0.5 * std::pow(sigL,2.0));     // estimate an M1 value based on M0 and lognormal distribution

    if (M[2] <= 0.0)
        M[2] = M[0] * std::pow(mavg,2) * exp(0.5 * 4 * std::pow(sigL,2.0));     // estimate an M2 value based on M0 and lognormal distribution
...
double sootModel_MOMIC::f_grid(int x, int y) {
    ...
    if (y<=2)       // 4 point interpolant
        fhalf = std::pow(f0,5./16.) * std::pow(f1,15./16.) * std::pow(f2, -5./16.) * std::pow(f3, 1./16.);
    else if (y<=3)  // 3 point interpolant
        fhalf = std::pow(f0, 3./8.) * std::pow(f1,  3./4.) * std::pow(f2,  -1./8.);
    else            // h point interpolant
        fhalf = std::pow(f0, 1./2.) * std::pow(f1,  1./2.);
...
double sootModel_MOMIC::g_grid(int y) {
    ...
    double ghalf;
    if (y<=2)       // 4 point interpolant
        ghalf = std::pow(g0,5./16.) * std::pow(g1,15./16.) * std::pow(g2, -5./16.) * std::pow(g3, 1./16.);
    else if (y<=3)  // 3 point interpolant
        ghalf = std::pow(g0, 3./8.) * std::pow(g1,  3./4.) * std::pow(g2,  -1./8.);
    else            // h point interpolant
        ghalf = std::pow(g0, 1./2.) * std::pow(g1,  1./2.);
...
void sootModel_MOMIC::set_mDimerPowers() {
    ...
    mD26  = std::pow(mDimer,  2./6.);
    mDn26 = std::pow(mDimer, -2./6.);
    mDn46 = std::pow(mDimer, -4./6.);

    mDn36  = std::pow(mDimer,  -3./6.);
    mDn16  = std::pow(mDimer,  -1./6.);
    mD16   = std::pow(mDimer,   1./6.);
    mD36   = std::pow(mDimer,   3./6.);
    mD56   = std::pow(mDimer,   5./6.);
    mD76   = std::pow(mDimer,   7./6.);
    mD96   = std::pow(mDimer,   9./6.);
    mD116  = std::pow(mDimer,  11./6.);
    mD136  = std::pow(mDimer,  13./6.);
    mD156  = std::pow(mDimer,  15./6.);
    mD176  = std::pow(mDimer,  17./6.);
    mD196  = std::pow(mDimer,  19./6.);
}
