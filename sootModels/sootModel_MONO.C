...
void sootModel_MONO::setSourceTerms(state &state) {
    ...
    if (tar->mechType != tarMech::NONE) {
        ...
        double Am2m3 = M0 > 0 ? M_PI*std::pow(6./(M_PI*rhoSoot)*M1/M0, twothird)*M0 : 0;
        ...
        if (tar->mechType == tarMech::AJ_RED) {
            ...
            double gasify = tar->getSurfaceTarRate(state);
            double rAN    = 2*CA*std::pow(6*gasSpMW[(int)gasSp::C]/(M_PI*rhoSoot), 1.0/6.0)*std::sqrt(6*kb*state.T/rhoSoot)*std::pow(M1/gasSpMW[(int)gasSp::C], 1.0/6.0)*std::pow(M0, 11.0/6.0); // soot aggregation #/m3*s
