...
double oxidationModel_HACA::getOxidationSootRate(const state &state) const {
    ...
    double fR2 = 1.0E10 * std::pow(state.T, 0.734) * exp(-1.43 / RT) * state.getGasSpC(gasSp::OH) / 1000;
    double rR2 = 3.68E8 * std::pow(state.T, 1.139) * exp(-17.1 / RT) * state.getGasSpC(gasSp::H2O) / 1000;
    double fR3 = 2.0E13 * state.getGasSpC(gasSp::H) / 1000;
    double fR4 = 8.00E7 * std::pow(state.T, 1.56) * exp(-3.8 / RT) * state.getGasSpC(gasSp::C2H2) / 1000;
    ...
    double ROH    = 0.13*state.getGasSpC(gasSp::OH)*       // kg/m2/s     = 1290*0.13*POH/sqrt(T) with POH in atm
                    std::sqrt(Rg*state.T/(2.0*M_PI*gasSpMW[(int)gasSp::OH]))*
                    gasSpMW[(int)gasSp::C];
    return RO2 + ROH;
}
