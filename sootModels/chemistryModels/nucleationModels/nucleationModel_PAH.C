...
double nucleationModel_PAH::getNucleationSootRate(state& state) {
    ...
    static double preFac = 4.0*std::sqrt(M_PI*kb*state.T)*   // compare to coag->getKfm; here no eps_c and FM_multiplier, and simp with m1=m2
                           std::pow(6./(M_PI*rhoSoot), twothird);
    ...
    for (int sp=0; sp<(int)pahSp::size; sp++) {
        ...
        double ndoti = pahSpGamma[sp]*preFac*std::pow(pahSpMW[sp]/Na, 1.0/6.0)*Ni*Ni;    // dn_pahi/dt
    ...
    double beta_DD = preFac*std::pow(mDimer, 1.0/6.0);       // dimer self-collision rate coefficient
    ...
    if (nDotD > 0 && mDimer > 0)     // nDimer initialized to 0 above
        nDimer = 2.0*nDotD/(I_beta_DS + std::sqrt(I_beta_DS*I_beta_DS + 4.*beta_DD*nDotD));   // #/m3
