...
double sootModel_LOGN::pahSootCollisionRatePerDimer(const state &state, const double mDimer) {
    ...
    const double mD16  = std::pow(mDimer,  1./6.);
    const double mDn16 = std::pow(mDimer, -1./6.);
    const double mDn36 = std::pow(mDimer, -0.5);
    const double mD26  = std::pow(mDimer,  onethird);
    const double mDn26 = std::pow(mDimer, -onethird);
    const double mD46  = std::pow(mDimer,  twothird);
    const double mDn46 = std::pow(mDimer, -twothird);
...
void sootModel_LOGN::setSourceTerms(state &state) {
    ...
    if (grow->mechType != growthMech::NONE) {
        ...
        G0 = 0;
        G1 = Kgrw*M_PI*std::pow(6./(M_PI*rhoSoot), twothird)*M46;
        G2 = Kgrw*M_PI*std::pow(6./(M_PI*rhoSoot), twothird)*M106* 2.0;
    }
    ...
    if (oxid->mechType != oxidationMech::NONE) {
        ...
        X0 =  0;
        X1 = -Koxi*M_PI*std::pow(6.0/(M_PI*rhoSoot), twothird)*M46;
        X2 = -Koxi*M_PI*std::pow(6.0/(M_PI*rhoSoot), twothird)*M106* 2.0;
    }
...
double sootModel_LOGN::Mr(double r, double M0, double M1, double M2) const {
    ...
    if ( (M0 <= 0.0 && M0_exp <= 0)  || (M1 <= 0.0 && M1_exp <= 0) || (M2 <= 0.0 && M2_exp <= 0) )
        ...
        return std::pow(M0, M0_exp) * std::pow(M1, M1_exp) * std::pow(M2, M2_exp);
